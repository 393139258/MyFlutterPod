...
@interface YMFlutterTexturePresenter()
...
@implementation YMFlutterTexturePresenter
...
- (instancetype)initWithImageStr:(id)imageStr size:(CGSize)size asGif:(Boolean)asGif {
    ...
    if (self){
		...
		[self loadImageWithImageType:imageStr];
    }
    return self;
}

- (void)loadImageWithImageType:(id)imageStr {
	if ([imageStr isKindOfClass:[NSString class]]) {
		...
		self.target = [self CVPixelBufferRefFromUiImage:imageStr size:self.size];
	}
}
...
-(void)loadImageWithStrForLocal:(NSString*)imageStr{
    if (self.asGif) {
		...
		NSLog(@"iamge = %@ - %@", imageStr, NSStringFromCGSize(self.size));
...
-(void)loadImageWithStrFromWeb:(NSString*)imageStr{
    __weak typeof(YMFlutterTexturePresenter*) weakSelf = self;
